...
export class HideCommand implements Command {
	public builder = new SlashCommandBuilder()
		.setName('hide')
		.setDescription(
			'Bir Byond hesabını web sitedeki arama sonuçlarından gizler.'
		)
	...
	public async execute(interaction: ChatInputCommandInteraction) {
		...
		if (statusCode === 200) {
			logger.info(`Ckey \`${ckey}\` arama sonuçlarından gizlendi`);
...
export class UnhideCommand implements Command {
	public builder = new SlashCommandBuilder()
		.setName('unhide')
		.setDescription(
			'Bir Byond hesabının web sitedeki arama sonuçlarından gizlenmesini kaldırır.'
		)
		.setDefaultMemberPermissions(PermissionFlagsBits.ManageRoles)
		.addStringOption((option) =>
			option
